...
def print_learning_info(info: dict):
    ...
    stat = '-----------------------------------'
    stat += "\n"
    stat += 'Learning Finished.'
    stat += "\n"
    stat += 'Learning Rounds:  {}'.format(info['learning_rounds'])
    stat += "\n"
    stat += 'Number of states: {}'.format(info['automaton_size'])
    stat += "\n"
    stat += 'Time (in seconds)'
    stat += "\n"
    stat += '  Total                : {}'.format(info['total_time'])
    stat += "\n"
    stat += '  Learning algorithm   : {}'.format(info['learning_time'])
    stat += "\n"
    stat += '  Conformance checking : {}'.format(info['eq_oracle_time'])
    stat += "\n"
    stat += 'Learning Algorithm'
    stat += "\n"
    stat += ' # Membership Queries  : {}'.format(info['queries_learning'])
    stat += "\n"

    if 'cache_saved' in info.keys():
        stat += ' # MQ Saved by Caching : {}'.format(info['cache_saved'])
        stat += "\n"

    stat += ' # Steps               : {}'.format(info['steps_learning'])
    stat += "\n"
    stat += 'Equivalence Query'
    stat += "\n"
    stat += ' # Membership Queries  : {}'.format(info['queries_eq_oracle'])
    stat += "\n"
    stat += ' # Steps               : {}'.format(info['steps_eq_oracle'])
    stat += "\n"
    stat += '-----------------------------------'
    stat += "\n"
