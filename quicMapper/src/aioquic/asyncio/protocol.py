...
class QuicConnectionProtocol(asyncio.DatagramProtocol):
    ...
    # EDITED here
    # FIXED !!!!!
    # Only start the timer when the client sends at least one datagram
    def transmit(self) -> None:
        ...
        """if (datagramNum > 0):
            if self._timer is None and timer_at is not None:
                # HEREE
                # callback handle_timer when reached the time/packet received from the server?
    ...
    # process the response send from the server
    def datagram_received(self, data: Union[bytes, Text], addr: NetworkAddress) -> None:
        if(not self.isLostResponse):
    ...
    # get server response here when timeout to resend lost packet
    ...
    # process events (thing that should do after receiving the response from the server) here
