...
# ADDED here for mapper error
class MapperError():
    NOT_ABLE_TO_WRITE_CERTIFICATE = 0
    NOT_ABLE_TO_WRITE_EMPTY_CERTIFICATE = 1
    NOT_ABLE_TO_WRITE_INVALID_CERTIFICATE = 2
    NOT_ABLE_TO_WRITE_CONNECTION_CLOSE = 3
    NOT_ABLE_TO_WRITE_NEW_CONNECTION_ID = 4
    NOT_ABLE_TO_WRITE_PING = 5
...
class QuicConnection:
    ...
    def __init__(
        ...
        *,
        ...
        set_able_send_0rtt_state_cb: Optional[tls.SetAbleSend0rttStateCallback] = None,
    ) -> None:
        ...
        self._local_active_connection_id_limit = 2 # EDITED HERE
        ...
        self._set_able_send_0rtt_state_cb = set_able_send_0rtt_state_cb
        ...
        self.mapperError = []
        self.closingState = []
        self.sendEpochType = None
        self.handshakeSent = False
        self.receivedToken = b""
        self.invalidToken = b""
        self.padAllInitialPackets = True
        self.padInitialACK = 1
        self.target = None
        self.isReceivedServerHello = False
        self.malformedPacketType = None
        self.send0rttAck = False
        self.isWriteAck = False
        #self.destConIDList: List[QuicConnectionId] = [] # to store peer Con ID # 0: original Destination Connection ID, 1: current Destination Connection ID
        #self.destConIDList.append(QuicConnectionId(cid=self._peer_cid.cid, sequence_number=1)) # append the original Destination Connection ID

        # ADDED HERE for getShortTimeout
        self.isGetShortTimeout = False
        self.startTime = None
        self.endTime = None
    ...
    # ADDED HERE
    # create new random connection ID for self._peer_cid
    # save the current peer id into the 
    #def createAndUseInvalidDestConID(self) -> None:
    #    self._peer_cid = QuicConnectionId(
    #        cid=os.urandom(self._configuration.connection_id_length), sequence_number=1
    #    )
    #    
    #    self.destConIDList.append(self._peer_cid)

    # ADDED HERE
    # change the current destination connection ID
    # index 0 - peer connection id that the client pick
    # idex 1 - peer connection id that the server pick
    #def changeDestConID(self, conIDIndex) -> None:
    #    #if(conIDIndex > -1 and conIDIndex < len(self.destConIDList)):
    #    if(len(self.destConIDList) > conIDIndex):
    #        self._peer_cid = QuicConnectionId(cid=self.destConIDList[conIDIndex].cid, sequence_number=self.destConIDList[conIDIndex].sequence_number)
        #self._peer_cid = QuicConnectionId(cid=b'\xb9\xd3U\xce;\xe9H\x1c', sequence_number=1)
    ...
    def close(
        ...
        if self._close_event is None and self._state not in END_STATES:
            # ADDED here
            if(sendEpochType is not None):
                if(self._cryptos[sendEpochType].send.is_valid()):
                    # When Epoch type is specified 
                    self.sendEpochType = sendEpochType
                    self._close_event = events.ConnectionTerminated(error_code=error_code, frame_type=frame_type, reason_phrase=reason_phrase,)
                    self._close_pending = True
                else:
                    # The epoch is not available yet
                    self.mapperError.append(MapperError.NOT_ABLE_TO_WRITE_CONNECTION_CLOSE)
                    raise Exception("Not able to send Connection Close in " + str(sendEpochType) + ".")
            else:
                if(sendEpochType is None and self.handshakeSent and not self._handshake_confirmed):
                    # When normal connection close, first handshake is send and handshake not yet confirmed
                    # To make sure it send handshake type connection close in this situation
                    self.sendEpochType = tls.Epoch.HANDSHAKE
                elif(sendEpochType is None and self.handshakeSent and self._handshake_confirmed):
                    self.sendEpochType = tls.Epoch.ONE_RTT
    ...
    def setupNetworkPathAndVersion(self, addr, version):
        self._network_paths = [QuicNetworkPath(addr, is_validated=True)]
    ...
    # ADDED here
    # set up connection id and idle timeout before first packet is send
    def initialize(self, now):
        # set idle timeout
        self._close_at = now + self._configuration.idle_timeout

        # re/initialize connection id, secrets
        self._initialize(self._peer_cid.cid)

    # ADDED here
    # include retry token in initial packet later
    def includeRetryToken(self):
        self._peer_token = self.receivedToken

    # ADDED here
    # include invalid (changing the last byte) retry token in Initial message later
    def includeInvalidRetryToken(self):
        receivedTokenLength = len(self.receivedToken)

        if(self.receivedToken[-1] != b"\xff"):
            self._peer_token = self.receivedToken[:receivedTokenLength-1] + b"\xff"
        else:
            self._peer_token = self.receivedToken[:receivedTokenLength-1] + b"\xfe"
    
    # ADDED here
    # change the next packet's Destination Connection ID field
    # only able to do this once the mapper received the first Initial packet from the server
    # only change after receiving server's hndFin and before sending hndFin 
    # because this is enough to know if we can use the original_destination_connection_id for the connection
    def changeDestinationConnectionID(self, isOriDestConID):
        # only change when client has received Server Hello from the server
        if(self.isReceivedServerHello):
            if(isOriDestConID and self._peer_cid.cid != self._original_destination_connection_id):
                self._peer_cid.cid = self._original_destination_connection_id
            # temporary do not change back during fuzzing because this will make the model messy.
            elif(not isOriDestConID and self._peer_cid.cid != self._remote_initial_source_connection_id):
                self._peer_cid.cid =  self._remote_initial_source_connection_id
            else:
                raise Exception("The connection ID is already changed.")
        else:
            raise Exception("No Connection ID to change.")

    # ADDED here
    # configure the padding for subseqeunce Initial packets
    def configureInitialPacketsPadding(self, isAddPadding):
        if(self.padAllInitialPackets and not isAddPadding):
            self.padAllInitialPackets = False
        elif(not self.padAllInitialPackets and isAddPadding):
            self.padAllInitialPackets = True
        elif(self.padAllInitialPackets and isAddPadding):
            raise Exception("Padding has been added previously.")
        elif(not self.padAllInitialPackets and not isAddPadding):
            raise Exception("Padding has been removed previously.")

    # ADDED here
    # exclude retry token in Initial packet later
    def excludeRetryToken(self):
        self._peer_token = b""

    # ADDED here
    # configure to send packet (Initial/Handshake) with empty payload
    def emptyPayloadPacket(self, sendEpochType):
        self.malformedPacketType = MalformedPacketType.EMPTY_PACKET
        self.sendEpochType = sendEpochType

    # ADDED here
    # configure to send packet (Initial/Handshake) with empty payload
    def unexpectedFrameTypePacket(self, sendEpochType):
        self.malformedPacketType = MalformedPacketType.UNEXPECTED_FRAME
        self.sendEpochType = sendEpochType

    # ADDED here
    # configure to send ACK in 0-RTT
    def ackFrame(self, sendEpochType):
        self.sendEpochType = sendEpochType

        if(sendEpochType == tls.Epoch.ZERO_RTT):
            self.send0rttAck = True

    # ADDED here
    # reset self.malformedPacketType and self.sendEpochType
    def resetMalformedPacket(self):
        self.malformedPacketType = None
        self.sendEpochType = None

    # ADDED here
    # ready and push Initial packet - clientHello
    def clientHello(self, cipherSuite):
        # get clientHello from TLS
        self.tls._client_send_hello(self._crypto_buffers[tls.Epoch.INITIAL], cipherSuite)
        self.sendEpochType = tls.Epoch.INITIAL
    ...
    # ADDED here
    # ready Initial packet - ping(probe)
    def probe(self, sendEpochType):
        if(sendEpochType is not None):
            # Send in the epoch specified
            if(self._cryptos[sendEpochType].send.is_valid()):
                self.sendEpochType = sendEpochType
                self._probe_pending = True
            else:
                # The epoch is not available yet
                self.mapperError.append(MapperError.NOT_ABLE_TO_WRITE_PING)
                raise Exception("Not able to send Ping in " + str(sendEpochType) + ".")
        else:
            if(sendEpochType is None and self.handshakeSent and not self._handshake_confirmed):
                # When normal connection close, first handshake is send and handshake not yet confirmed
                # To make sure it send handshake type connection close in this situation
                self.sendEpochType = tls.Epoch.HANDSHAKE
            elif(self._handshake_confirmed):
                # set to 1-RTT when handshake is confirmed
                self.sendEpochType = tls.Epoch.ONE_RTT
            else:
                # set to Initial
                self.sendEpochType = tls.Epoch.INITIAL
    ...
    # ADDED here
    # ready to send client certificate
    def sendValidClientCertificate(self):
        res = self.tls.clientSendCertificate(tls.CertType.VALID_CERTIFICATE, 
                                        self._crypto_buffers[tls.Epoch.HANDSHAKE])
        if(res):
            self._push_crypto_data()
        else:
            self.mapperError.append(MapperError.NOT_ABLE_TO_WRITE_CERTIFICATE)
            raise Exception("Not able to write certificate.")

    # ADDED here
    # ready to send empty client cert
    def sendEmptyClientCertificate(self):
        res = self.tls.clientSendCertificate(tls.CertType.EMPTY_CERTIFICATE, 
                                        self._crypto_buffers[tls.Epoch.HANDSHAKE])
        if(res):
            self._push_crypto_data()
        else:
            self.mapperError.append(MapperError.NOT_ABLE_TO_WRITE_EMPTY_CERTIFICATE)
            raise Exception("Not able to write empty certificate.")

    # ADDED here
    # send invalid client certificate
    def sendInvalidClientCertificate(self):
        res = self.tls.clientSendCertificate(tls.CertType.INVALID_CERTIFICATE, 
                                        self._crypto_buffers[tls.Epoch.HANDSHAKE])
        if(res):
            self._push_crypto_data()
        else:
            self.mapperError.append(MapperError.NOT_ABLE_TO_WRITE_INVALID_CERTIFICATE)
            raise Exception("Not able to write invalid certificate.")

    # ADDED here
    # ready to send client cert verify
    def sendClientCertificateVerify(self):
        self.tls.clientSendCertificateVerify(self._crypto_buffers[tls.Epoch.HANDSHAKE])
        self._push_crypto_data()

    # ADDED here
    # ready to send finished(), default packet = Handshake
    def sendFinished(self, epoch=tls.Epoch.HANDSHAKE):
        self.sendEpochType = epoch
        self.tls.clientSendFinished(self._crypto_buffers[epoch])
        self._push_crypto_data()

        # update current epoch and send NCID
        if not self._handshake_complete and self.tls.state in [
            tls.State.CLIENT_POST_HANDSHAKE,
            tls.State.SERVER_POST_HANDSHAKE,
            tls.State.CLIENT_PROCESSED_SESSION_TICKET,
        ]:
    ...
    # ADDED here
    # send the right amount of connection ID
    def sendValidNCID(self):
        self._replenish_connection_ids()

    # ADDED here (To-do)
    # send the wrong amount of connection ID
    def sendInvalidNCID(self):
        """
        Generate invalid connection IDs. (set and send more than 8 NCID)
        """
        while len(self._host_cids) < max(8+1, self._remote_active_connection_id_limit + 1):
    ...
    # ADDED here
    def resetSendEpochType(self):
        self.sendEpochType = None
    
    # ADDED here
    # reset the close event if the connection is still active after CC packet is sent
    def resetCloseEvent(self):
        self._close_event = None

    # ADDED here
    # reset padInitialACK after sending the Initial packet
    def resetPadInitialACK(self):
        self.padInitialACK = 1

    # ADDED here
    # close the mapper side connection if the connection is closed after CC packer is sent
    def closeMapperConnection(self, now):
        self._close_begin(is_initiator=True, now=now)

    # ADDED here
    # clear the peerResponse,mapperError,closingState lists at the end of getResponse in mapper.py
    def clearLists(self):
        self.peerResponse.clear()
        self.mapperError.clear()
        self.closingState.clear()
    ...
    def datagrams_to_send(self, now: float) -> List[Tuple[bytes, NetworkAddress]]:
        ...
        builder = QuicPacketBuilder(
            ...
            padAllInitialPackets=self.padAllInitialPackets,
            padInitialACK=self.padInitialACK,
            malformedPacketType=self.malformedPacketType,
        )
        if self._close_pending:
            epoch_packet_types = []
            
            # ADDED here to send 0-rtt connection close
            if(self.sendEpochType == tls.Epoch.ZERO_RTT and self._cryptos[tls.Epoch.ZERO_RTT].send.is_valid()):
                crypto = self._cryptos[self.sendEpochType]

                builder.start_packet(PACKET_TYPE_ZERO_RTT, crypto)

                self._write_connection_close_frame(
                    builder=builder,
                    epoch=tls.Epoch.ZERO_RTT,
                ...
                # EDITED here to only send the specific packet type with Connection Close 
                if(not self._handshake_confirmed or
                self.sendEpochType == tls.Epoch.INITIAL or
                self.sendEpochType == tls.Epoch.HANDSHAKE):
                    epoch_packet_types += [
                ...
                for epoch, packet_type in epoch_packet_types:
                    if(self.sendEpochType == epoch or self.sendEpochType == None):
            ...
            # DELETED HERE
            # Remove here to disable congestion control (solve non-deterministic in neqo B,BWR), See notes on 8/12/2022
            # congestion control
            ...
            # EDITED here
            # once a packet has an ACK, flush it without writting the other packet so that
            # there is no 2 different ACK (Initial ACK and Handshake ACK) in the same UDP datagram
            try:
                if(self.malformedPacketType != None):
                    self._write_malformed_packet(builder, self.sendEpochType, now)
                else:
                    if(not self._handshake_confirmed or self.sendEpochType == tls.Epoch.INITIAL or 
                    self.sendEpochType == tls.Epoch.HANDSHAKE):
                        for epoch in [tls.Epoch.INITIAL, tls.Epoch.HANDSHAKE]:
                            self._write_handshake(builder, epoch, now)

                    # EDITED here to send 0-RTT even after 1-RTT is used
                    if(self.sendEpochType == tls.Epoch.ZERO_RTT):
                        self._write_application(epoch=tls.Epoch.ZERO_RTT, builder=builder, network_path=network_path, now=now)

                    self._write_application(epoch=tls.Epoch.ONE_RTT, builder=builder, network_path=network_path, now=now)
                    #self._write_application(epoch=tls.Epoch.ZERO_RTT, builder=builder, network_path=network_path, now=now)
    ...
    def receive_datagram(self, data: bytes, addr: NetworkAddress, now: float) -> None:
        ...
        while not buf.eof():
            ...
            if (
                ...
                # ADDED here to append version negotiation packet to the peerResponse.
                self.peerResponse.append("VERSION_NEGOTIATION")
                ...
                # REMOVE HERE so that the mapper will not send client hello automatically
                #self._connect(now=now) 
                return
            elif (
            ...
            if header.packet_type == PACKET_TYPE_RETRY:
                # ADDED here
                self.peerResponse.append("RETRY")
                ...
                if (
                    ...
                    # DELETED here
                    # Do not store the token in self._peer_token
                    #self._peer_token = header.token

                    # ADDED here
                    # store the token in a new variable and copy to self._peer_token later when I want
                    self.receivedToken = header.token
                    ...
                    # reinitialize the secrets to prevent decryption failed (MUST)
            ...
            if self._peer_cid.sequence_number is None:
                # EDITED HERE  (not used anymore)
                # to test if the server accepts Destination Connection ID that does not match to the one it picked
                #self._peer_cid.cid = self.original_destination_connection_id # testing
                self._peer_cid.cid = header.source_cid # original
                self._peer_cid.sequence_number = 0
                
                # ADDED HERE  (not used anymore)
                # store the current Destination Connection ID
                #if (len(self.destConIDList) > 1):
                #    if(self.destConIDList[0].cid != self._peer_cid.cid):
                #        self.destConIDList.append(QuicConnectionId(cid=self._peer_cid.cid, sequence_number=self._peer_cid.sequence_number))
                #print(self.destConIDList)
            ...
            # Mapper will close the connection itself here when QuicConnectionError
    ...
    def _consume_peer_cid(self) -> None:
        ...
        """
        # ADDED HERE 
        # so that Mapper will remain using original destination connection id even retiring and consuming connection id
        if(self._peer_cid.cid == self._original_destination_connection_id):
            self._peer_cid_available.pop(0)
        else:
    ...
    def _close_begin(self, is_initiator: bool, now: float) -> None:
        ...
        if is_initiator:
            ...
            self.closingState.append("Connection closing.")
        else:
            self._set_state(QuicConnectionState.DRAINING)
            self.closingState.append("Connection draining.")
    ...
    def _close_end(self) -> None:
        ...
        self.closingState.append("Connection terminated.")
    ...
    def _initialize(self, peer_cid: bytes) -> None:
        ...
        # ADDED here
        # set invalid certificate in tls
        self.tls.invalid_certificate = self.configuration.invalid_certificate
        self.tls.invalid_certificate_chain = self.configuration.invalid_certificate_chain
        ...
        # ADDED here to set the send 0rtt state call back
        if self._set_able_send_0rtt_state_cb is not None:
            self.tls.set_able_send_0rtt_state_cb = self._set_able_send_0rtt_state_cb
        ...
        self._crypto_buffers = {
            ...
            # ADDED here to send CRYPTO frame in 0-RTT
            tls.Epoch.ZERO_RTT: Buffer(capacity=CRYPTO_BUFFER_SIZE),
        }
    ...
    def _handle_connection_close_frame(
        ...
        for response in self.peerResponse:
            if("CONNECTION_CLOSE" in str(response)):
                hasCCResponse = True
                break
        
        if(not hasCCResponse):
            self.peerResponse.append("CONNECTION_CLOSE (" + str(error_code) + ", " + reason_phrase + ")")
    ...
    def _handle_crypto_frame(
        ...
        if event is not None:
            ...
            try:
                ...
                # ADDED here
                for response in self.tls.peerResponse:
                    self.peerResponse.append(response)
                    
                    # ADDED here 
                    # Mapper can start to change the packet's DCID when CHANGE_DESTINATION_CONNECTION_ID_ORIGINAL is given
                    if(response == tls.HandshakeType.SERVER_HELLO and not self.isReceivedServerHello):
                        self.isReceivedServerHello = True
                    
                    # ADDED here to get the short timeout automatically
                    if(self.isGetShortTimeout and response == tls.HandshakeType.FINISHED):
                        self.endTime = time.perf_counter()

                self.tls.peerResponse.clear()            
    ...
    def _handle_handshake_done_frame(
        ...
        # ADDED HERE
        # for testing if the server still accept original destination connection ID after handshake is done.
        #self.changeDestConID(0)

        # for clients, the handshake is now confirmed
        if not self._handshake_confirmed:
            # DELETED here so that handshake secrets are not discarded
            #self._discard_epoch(tls.Epoch.HANDSHAKE)
            self._handshake_confirmed = True
            self._loss.peer_completed_address_validation = True

            # ADDED here (if append twice means the server has some problem)
            # only add here to prevent non-deterministic 
            # do not append more than 2 times to prevent non-deterministic (sometimes 4 and sometimes 3)
            if(self.peerResponse.count("HANDSHAKE_DONE") < 2):
                self.peerResponse.append("HANDSHAKE_DONE")
    ...
    def _handle_new_connection_id_frame(
        ...
        # ADDED HERE to prevent exception when reusing the connection id with seq no.= None
        if(self._peer_cid.sequence_number != None):
        ...
        # EDITED HERE to prevent accepting extra CID from msquic, temporary remove that statement.
        # print("Process Connection ID...")
        if sequence_number not in self._peer_cid_sequence_numbers:# and (1 + len(self._peer_cid_available) < self._local_active_connection_id_limit):
            # ADDED here
            self.peerResponse.append("NEW_CONNECTION_ID (" + str(hexdump(connection_id)) + ")")
            # print("Append Connection ID...")
        ...
        for quic_connection_id in retire:
            ...
            # ADDED HERE (remove the output if we retire extra CID)
            try:
                if("NEW_CONNECTION_ID" in self.peerResponse[-1]):
                    self.peerResponse.pop(-1)
            except Exception as e:
                pass

        # assign new CID if we retired the active one (only do this when we have enough available CID)
        if change_cid and len(self._peer_cid_available) > 0:
            self._consume_peer_cid()
    ...
    def _handle_new_token_frame(
        ...
        # ADDEDE here
        # if Mapper receives more than one token, counted as one to prevent non-deterministic
        if(self.peerResponse and "NEW_TOKEN" not in self.peerResponse):
            self.peerResponse.append("NEW_TOKEN")
    ...
    # EDITED here
    # append ping ack event with uid of -1 when probe is acked
        ...
        if delivery == QuicDeliveryState.ACKED:
            ...
            if uids:
                for uid in uids:
                    ...
                    self.peerResponse.append("PING.ACKED (" + str(uid) + ").")
            else:
                self._events.append(events.PingAcknowledged(-1))
                self.peerResponse.append("PING.ACKED")
        else:
    ...
    def _payload_received(
        ...
        while not buf.eof():
            ...
            # ADDED HERE to prevent the client sends a CC due to the issue in Quant itself. (only observe this behaviour on Quant)
            if(self.target != Server.QUANT):
            ...
            try:
                ...
                pass
            # ADDED HERE so that the Mapper will not close the connection itself when QuicConnectionError
            except QuicConnectionError as exc:
                self._logger.warning(exc)
                pass
    ...
    # EDITED HERE to prevent the Mapper sends ping automatically 
    def _send_probe(self) -> None:
        self._probe_pending = False

    # check here (can do something with this)
    def _parse_transport_parameters(
        ...
        if not from_session_ticket:
            if (
                ...
                # ADDED HERE to get msquic current Con ID
                #self.secondDestConID.cid = quic_transport_parameters.initial_source_connection_id
                raise QuicConnectionError(
                    error_code=QuicErrorCode.TRANSPORT_PARAMETER_ERROR,
                    frame_type=QuicFrameType.CRYPTO,
        ...
        if not from_session_ticket:
            ...
            if (
                ...
                # update the stateless_reset_token with the new peer id
    ...
    def _write_application(
        self, epoch: tls.Epoch, builder: QuicPacketBuilder, network_path: QuicNetworkPath, now: float
    ) -> None:
        crypto_stream: Optional[QuicStream] = None
        if(self._cryptos[tls.Epoch.ONE_RTT].send.is_valid() and epoch == tls.Epoch.ONE_RTT):
            
            # EDITED HERE for experiments (try to send Application packet with Initial keys)
        ...
        elif(self._cryptos[tls.Epoch.ZERO_RTT].send.is_valid() and epoch == tls.Epoch.ZERO_RTT):
            crypto = self._cryptos[tls.Epoch.ZERO_RTT]
            crypto_stream = self._crypto_streams[tls.Epoch.ZERO_RTT]
        ...
        while True:
            ...
            if(packet_type == PACKET_TYPE_ZERO_RTT and self.send0rttAck):
                self._write_ack_frame(builder=builder, space=space, now=now, isMalformedAck=True)
                self.send0rttAck = False
            elif(self._handshake_complete):
                # ACK
                # non-deterministic here
                # EDITED here, space.ack_at -1
                if space.ack_at is not None and space.ack_at -1 <= now:
    ...
    # ADDED HERE
    # create Long Header Packets (Initial and Handshake) with empty payload
    # create Long Header Packets (Initial and Handshake) with unexpected frames
    def _write_malformed_packet(self, builder: QuicPacketBuilder, epoch: tls.Epoch, now: float) -> None:
        crypto = self._cryptos[epoch]
        ...
        elif  epoch == tls.Epoch.HANDSHAKE:
            packet_type = PACKET_TYPE_HANDSHAKE
        elif epoch == tls.Epoch.ZERO_RTT:
            packet_type = PACKET_TYPE_ZERO_RTT
    ...
    # EDITED HERE
    # create packet based on self.sendEpochType
    def _write_handshake(
        ...
        while True:
            ...
            builder.start_packet(packet_type, crypto, new_packet=True)
            ...
            # ADDED here
            if(self.sendEpochType == epoch and self._probe_pending):
                self._write_ping_frame(builder, comment="probe")
                self._probe_pending = False
                self.sendEpochType = None
    ...
    # EDITED here to send normal ACK and malformed ACK
    def _write_ack_frame(
        self, builder: QuicPacketBuilder, space: QuicPacketSpace, now: float, isMalformedAck: bool=False
    ) -> None:
        if(not isMalformedAck):
            # calculate ACK delay
            ...
            try:
                ...
                invalidAckDelayEncoded = int(1 * 1000000) >> self._local_ack_delay_exponent # hardcode delay of 1
                malformedRangeSet = RangeSet()
                malformedRangeSet.add(1,15) # hardcode 1-15 ack range
                push_ack_frame(buf, malformedRangeSet, invalidAckDelayEncoded)
            except Exception as e:
                print(e)

        self.isWriteAck = True
