...
HeaderProtection_apply(HeaderProtectionObject *self, PyObject *args)
{
    ...
    // EDITED here
    // because we already use 4 bytes of packet number, the sample size is starting from the payload itself 
    // previosuly: res = HeaderProtection_mask(self, payload + PACKET_NUMBER_LENGTH_MAX - pn_length);
    res = HeaderProtection_mask(self, payload );
    CHECK_RESULT(res != 0);

    memcpy(self->buffer, header, header_len);
    memcpy(self->buffer + header_len, payload, payload_len);

    // here to put the mask output to the correct bits/bytes
    // The least significant bits of the first byte of the packet are masked by the least significant bits of the first mask byte
