...
class State(Enum):
    ...
    CLIENT_RECEIVED_FINISHED = 6.5  # ADDED here
    CLIENT_POST_HANDSHAKE = 7
    CLIENT_PROCESSED_SESSION_TICKET = 8
...
class CertType:
    ...
    EMPTY_CERTIFICATE = "empty"
    INVALID_CERTIFICATE = "invalid"
...
def pull_certificate_request(buf: Buffer) -> CertificateRequest:
    ...
    with pull_block(buf, 3):
        certReq.request_context = pull_opaque(buf, 1)
        ...
        def pull_extension() -> None:
            ...
            # only pull when the extension length is more than 0
            if (extension_length > 0):
                if extension_type == ExtensionType.SIGNATURE_ALGORITHMS:
                    certReq.signature_algorithms = pull_list(buf, 2, buf.pull_uint16)
                else:
                    # ADDED HERE
                    # for quinn (extra ca extension)
                    ca_bytes = buf.pull_uint16()
                    buf.pull_bytes(ca_bytes)
...
SetAbleSend0rttStateCallback = Callable[[State], None]
...
class Context:
    def __init__(
        ...
        # ADDED here
        # var to store invalid certificate
        self.invalid_certificate: Optional[x509.Certificate] = None
        self.invalid_certificate_chain: List[x509.Certificate] = []
        ...
        self.set_able_send_0rtt_state_cb: Optional[SetAbleSend0rttStateCallback] = None
        ...
        # ADDED here
        self.peerResponse = []
        self.tempNextEncKey = None
        self.certReq = None
        self.updated_key_schedule = False
        self.extracted_session_ticket = None
        self.psk_binder_key = None
        self.psk_binder_length = None
        self.psk_binder = None
    ...
    def handle_message(
        ...
        #     self._client_send_hello(output_buf[Epoch.INITIAL], cipherSuiteName="")
        #     return
        ...
        while len(self._receive_buffer) >= 4:
            ...
            # save peer response
            self.peerResponse.append(HandshakeType(message_type))
            # client states
            if self.state == State.CLIENT_EXPECT_SERVER_HELLO:
                ...
                if message_type == HandshakeType.CERTIFICATE:
                    ...
                    # ADDED here
                    self._client_handle_certificate_request(input_buf, output_buf[Epoch.HANDSHAKE])
                else:
                ...
                if message_type == HandshakeType.FINISHED:
                    ...
                    raise AlertUnexpectedMessage
            elif self.state == State.CLIENT_RECEIVED_FINISHED:
                if message_type == HandshakeType.NEW_SESSION_TICKET:
                    self._client_extract_new_session_ticket(input_buf)
    ...
    # EDITED HERE for testing different cipher
    def _client_send_hello(self, output_buf: Buffer, cipherSuiteName) -> None:
        ...
        # ADDED HERE to test with different cipher
        cipherSuite = []

        if(cipherSuiteName == "AES_128_GCM_SHA256"):
            cipherSuite.append(CipherSuite.AES_128_GCM_SHA256)
        elif(cipherSuiteName == "AES_256_GCM_SHA384"):
            cipherSuite.append(CipherSuite.AES_256_GCM_SHA384)
        elif(cipherSuiteName == "CHACHA20_POLY1305_SHA256"):
            cipherSuite.append(CipherSuite.CHACHA20_POLY1305_SHA256)
        else:
            cipherSuite = [int(x) for x in self._cipher_suites]
        ...
        if self.session_ticket and self.session_ticket.is_valid:
            # install the psk key and update the hash if it has not been done
            if(self.psk_binder_key == None and self.psk_binder_length == None and self.psk_binder == None):
                ...
                self.psk_binder_key = self._key_schedule_psk.derive_secret(b"res binder")
                self.psk_binder_length = self._key_schedule_psk.algorithm.digest_size
                ...
                hello.pre_shared_key = OfferedPsks(
                    ...
                    binders=[bytes(self.psk_binder_length)],
                )
                ...
                # calculate binder
                hash_offset = tmp_buf.tell() - self.psk_binder_length - 3
                self._key_schedule_psk.update_hash(tmp_buf.data_slice(0, hash_offset))
                self.psk_binder = self._key_schedule_psk.finished_verify_data(self.psk_binder_key)
                hello.pre_shared_key.binders[0] = self.psk_binder
                self._key_schedule_psk.update_hash(
                    tmp_buf.data_slice(hash_offset, hash_offset + 3) + self.psk_binder
                )
                ...
                # if the psk key is installed and hash is updated, only modify the client hello message
                # update hello
                if self.session_ticket.max_early_data_size is not None:
                    hello.early_data = True
                hello.pre_shared_key = OfferedPsks(
        ...
        # tell the Mapper to start sending 0-RTT packets 
        if self.set_able_send_0rtt_state_cb is not None:
            self.set_able_send_0rtt_state_cb(State.CLIENT_EXPECT_SERVER_HELLO)
    ...
    def _client_handle_certificate_request(self, input_buf: Buffer, output_buf: Buffer) -> None:
        ...
        # extract data from cert_request for later use
        self.certReq = pull_certificate_request(input_buf)
    ...
    def _client_handle_finished(self, input_buf: Buffer, output_buf: Buffer) -> None:
        ...
        self.tempNextEncKey = None
        self.tempNextEncKey = self.key_schedule.derive_secret(b"c ap traffic")

        # ADDED this line
        self.updated_key_schedule = True

        # ADDED here to know when the mapper can change connection id
        self._set_state(State.CLIENT_RECEIVED_FINISHED)

        # send finished (origin)
    ...
    # ADDED here
    # send valid, empty, invalid certificate to server
    def clientSendCertificate(self, certType, output_buf: Buffer):
        if(self.updated_key_schedule):
            if(certType == CertType.VALID_CERTIFICATE):
                with push_message(self.key_schedule,output_buf):
                    push_certificate(
                        output_buf,
                        Certificate(
            ...
            elif(certType == CertType.EMPTY_CERTIFICATE):
                with push_message(self.key_schedule,output_buf):
                    push_certificate(
                        output_buf,
                        Certificate(
            ...
            elif(certType == CertType.INVALID_CERTIFICATE):
                with push_message(self.key_schedule,output_buf):
                    push_certificate(
                        output_buf,
                        Certificate(
                            ...
                            certificates=[
                                (x.public_bytes(Encoding.DER), b"")
                                for x in [self.invalid_certificate] + self.invalid_certificate_chain
                            ],
                        ),
                    )
            return True
    ...
    # ADDED here
    # send certificate verify to server
    def clientSendCertificateVerify(self, output_buf: Buffer):
        
        # fixed signature algorithms here
        ...
        # need to get the right signature algorithms
        signature_algorithm = negotiate(
            signature_algorithms,
            self.certReq.signature_algorithms,
    ...
    def clientSendFinished(self, output_buf: Buffer):
        with push_message(self.key_schedule, output_buf):
            push_finished(
                output_buf,
                Finished(
        ...
        # commit traffic key
        self._enc_key = self.tempNextEncKey
        self.update_traffic_key_cb(
            Direction.ENCRYPT,
            Epoch.ONE_RTT,
            self.key_schedule.cipher_suite,
        ...
        if (self.extracted_session_ticket != None):
            self._client_build_new_session_ticket()
            self.extracted_session_ticket = None
    
    # ADDED here
    # extract the session ticket and build it later
    # so that Mapper uses the correct transcript hash (computed with all the messages from Client Hello to Client Finished)
    def _client_extract_new_session_ticket(self, input_buf: Buffer) -> None:
        self.extracted_session_ticket = pull_new_session_ticket(input_buf)

    # ADDED here
    # only call this after sending/prepare finished message after the Mapper has the correct transcript hash
    def _client_build_new_session_ticket(self) -> None:
        # notify application
        if self.new_session_ticket_cb is not None:
            ticket = self._build_session_ticket(
                self.extracted_session_ticket, self.received_extensions
            )
            self.new_session_ticket_cb(ticket)

            self._set_state(State.CLIENT_PROCESSED_SESSION_TICKET)
